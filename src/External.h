...
// Ensure exactly one JSON style is defined
#if defined(OLD_STYLE_JSON) && defined(NEW_STYLE_JSON)
#error "Cannot define both OLD_STYLE_JSON and NEW_STYLE_JSON"
#endif
#if !defined(OLD_STYLE_JSON) && !defined(NEW_STYLE_JSON)
#error "Must define either OLD_STYLE_JSON or NEW_STYLE_JSON"
#endif

// ICriticalSection and types provided via PCH.h
...
template <> inline SInt32 ParseValue<SInt32>(Value value, SInt32 missing) { return (value.isNull() ||
!value.isConvertibleTo(Json::intValue)) ? missing : value.asInt(); } template <> inline float ParseValue<float>(Value value, float missing)
{ return (value.isNull() || !value.isConvertibleTo(Json::realValue)) ? missing : value.asFloat(); } template <> inline BSFixedString
ParseValue<BSFixedString>(Value value, BSFixedString missing) { return value.isString() ? BSFixedString(value.asCString()) :
(!value.isNull() && value.isConvertibleTo(Json::stringValue)) ? BSFixedString(value.asString().c_str()) : missing;
}
...
template <typename T> inline T ParseValue(Value value, T missing) {
  static_assert(sizeof(T) == 0, "ParseValue not specialized for this type");
  return missing;
}
...
template <> inline BSFixedString ParseValue<BSFixedString>(Value value, BSFixedString missing) {
  return value.isString() ? BSFixedString(value.asCString()) : missing;
}
template <> inline TESForm *ParseValue<TESForm *>(Value value, TESForm *missing) {
  return value.isNull() ? missing : Forms::ParseFormString(value.asString());
}

template <typename T> inline T ParseValue(Value value) {
  T t = T();
  return ParseValue<T>(value, t);
}
...
public:
  ...
  // Non-copyable due to ICriticalSection member
  ExternalFile(const ExternalFile &) = delete;
  ExternalFile &operator=(const ExternalFile &) = delete;

  // Non-movable (ICriticalSection doesn't support move)
  ExternalFile(ExternalFile &&) = delete;
  ExternalFile &operator=(ExternalFile &&) = delete;

  ~ExternalFile() = default;
  ...
  inline Value Resolve(const std::string &pathto); // { Path path(pathto.front() != '.' ? '.' + pathto : pathto); return path.resolve(root,
                                                   // Value(Json::nullValue)); }
  inline Value Resolve(const std::string &pathto,
                       Value missing); // { Path path(pathto.front() != '.' ? '.' + pathto : pathto); return path.resolve(root, missing); }
