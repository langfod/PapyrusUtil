...
static ICriticalSection *s_loadLock = nullptr;
static FileVector *s_Files = nullptr;
static std::once_flag s_globalsInitFlag;
...
inline Value ExternalFile::make(SInt32 v) const { return Value::Int(v); }
inline Value ExternalFile::make(float v) const { return Value(v); }
inline Value ExternalFile::make(BSFixedString v) const { return Value(v.data()); }
inline Value ExternalFile::make(TESForm *v) const { return Value(Forms::GetFormString(v)); }
...
inline BSFixedString ExternalFile::parse(Value value, BSFixedString missing) const {
  return value.isString()                                                ? BSFixedString(value.asCString())
         : (!value.isNull() && value.isConvertibleTo(Json::stringValue)) ? BSFixedString(value.asString().c_str())
                                                                         : missing;
}

template <typename T> inline bool ExternalFile::HasKey(const std::string &key) {
  return root.isMember(Type<T>()) && root[Type<T>()].isMember(key);
}
...
template <typename T> void ExternalFile::SetValue(std::string key, T value) {
  s_dataLock.Enter();
  boost::to_lower(key);
...
template <typename T> T ExternalFile::GetValue(std::string key, T value) {
  s_dataLock.Enter();
  boost::to_lower(key);
  if (HasKey<T>(key))
...
template <typename T> bool ExternalFile::UnsetValue(std::string key) {
  bool removed = false;
  s_dataLock.Enter();
...
template <typename T> bool ExternalFile::HasValue(std::string key) {
  bool has = false;
  s_dataLock.Enter();
  boost::to_lower(key);
...
SInt32 ExternalFile::AdjustValue(std::string key, SInt32 value) {
  ...
  boost::to_lower(key);
  SInt32 result = value;
  if (HasKey<SInt32>(key))
    result = value + root[Type<SInt32>()][key].asInt();
  root[Type<SInt32>()][key] = Value::Int(result);
  isModified = true;
  s_dataLock.Leave();
  return result;
}
...
float ExternalFile::AdjustValue(std::string key, float value) {
  ...
  float result = value;
  if (HasKey<float>(key))
    result = value + root[Type<float>()][key].asFloat();
  root[Type<float>()][key] = Value(result);
  isModified = true;
  s_dataLock.Leave();
  return result;
}
...
Value ExternalFile::AdjustValue(std::string type, std::string key, Value value) {
  ...
  if (HasKey(type, key)) {
    if (value.isInt())
      value = Value::Int(value.asInt() + root[type][key].asInt());
    else
      value = Value(value.asFloat() + root[type][key].asFloat());
  }
  root[type][key] = value;
  isModified = true;
  s_dataLock.Leave();
...
int ExternalFile::ListAdd(std::string type, std::string key, Value value, bool allowDuplicate) {
  s_dataLock.Enter();
  boost::to_lower(key);
  // Check duplicate inside the lock to avoid TOCTOU
  if (!allowDuplicate && HasKey(type, key)) {
  ...
  int size = static_cast<int>(root[type][key].size() - 1);
  isModified = true;
  s_dataLock.Leave();
  return size;
}
...
int ExternalFile::ListRemove(std::string type, std::string key, Value removing, bool allInstances) {
  ...
  if (HasKey(type, key)) {
    ...
    for (Value::iterator itr = root[type][key].begin(); itr != root[type][key].end(); ++itr) {
      if (!allInstances && removed > 0)
        list.append((*itr));
      else if (removing != (*itr))
        list.append((*itr));
      else
        removed += 1;
    }
...
int ExternalFile::ListCountValue(std::string type, std::string key, Value value, bool exclude) {
  ...
  if (HasKey(type, key)) {
    ...
    if (exclude)
      count = (root[type][key].size() - count);
  }
  s_dataLock.Leave();
  return count;
}
...
int ExternalFile::ListResize(std::string type, std::string key, int length, Value filler) {
  if (length == 0)
    return ListClear(type, key) * -1;
  s_dataLock.Enter();

  int start = 0;
  boost::to_lower(key);
  if (HasKey(type, key)) {
    start = static_cast<int>(root[type][key].size());
    if (length < start)
      root[type][key].resize(length);
    else {
      while (static_cast<int>(root[type][key].size()) < length && root[type][key].size() < 500)
  ...
  s_dataLock.Leave();
  return static_cast<int>(root[type][key].size()) - start;
}

template <typename T> T ExternalFile::ListAdjust(std::string key, int index, T adjustBy) {
  s_dataLock.Enter();
  boost::to_lower(key);
...
template <typename T> void ExternalFile::ListSlice(std::string key, VMArray<T> Output, int startIndex) {
  if (Output.size() > 0) {
    ...
    if (HasKey(type, key)) {
      ...
      for (UInt32 index = 0; index < length && itr != root[type][key].end(); ++itr, ++index) {
...
template <typename T> bool ExternalFile::ListCopy(std::string key, VMArray<T> Input) {
  if (Input.size() > 0) {
    ...
    for (UInt32 i = 0; i < length; ++i) {
      root[type][key].append(MakeValue<T>(Input[i]));
    }
    isModified = true;
...
template <typename T> VMResultArray<T> ExternalFile::ToArray(std::string key) {
  VMResultArray<T> arr;
  s_dataLock.Enter();
...
int ExternalFile::CountPrefix(std::string type, std::string prefix) {
  ...
  if (root.isMember(type) && !root[type].empty()) {
    ...
    for (Value::Members::iterator itr = members.begin(); itr != members.end(); ++itr) {
      if (boost::starts_with((*itr), prefix))
        count++;
    }
  }
  s_dataLock.Leave();
...
static boost::random::mt19937 rnd{static_cast<uint32_t>(std::random_device{}())};
static std::mutex rndMutex;
...
inline Value ExternalFile::Resolve(const std::string &pathto) {
  Path path(pathto.front() != '.' ? '.' + pathto : pathto);
  return path.resolve(root, Value(Json::nullValue));
}
inline Value ExternalFile::Resolve(const std::string &pathto, Value missing) {
  Path path(pathto.front() != '.' ? '.' + pathto : pathto);
  return path.resolve(root, missing);
}
  ...
  if (!path.empty()) {
    ...
    if (!value.isNull()) {
      ...
      case 9:
        ...
        break;
      default:
        logger::warn("ClearPath: Path depth {} exceeds maximum supported depth of 9", args.size());
        s_dataLock.Leave();
        return;
      }
      ...
      logger::info("ClearPath: Unable to resolve {}", path.c_str());
    }
    s_dataLock.Leave();
  }
}
  ...
  if (!path.empty()) {
    ...
    if (!pathArray.isNull() && pathArray.isArray()) {
      ...
      case 9:
        ...
        break;
      default:
        logger::warn("ClearPathIndex: Path depth {} exceeds maximum supported depth of 9", args.size());
        s_dataLock.Leave();
        return;
      }

      if (isModified)
        logger::info("ClearPathIndex: Removed Index {} from array {}", idx, path.c_str());
      else
        logger::info("ClearPathIndex: FAILED. Index was likely out of range.");

    } else {
      logger::info("ClearPathIndex: Unable to resolve an array at {}", path.c_str());
    }
    s_dataLock.Leave();
  }
}
...
template <typename T> void ExternalFile::SetPathArray(const std::string &path, VMArray<T> Input, bool append) {
  if (!path.empty()) {
    ...
    // Normalize path with leading dot
    std::string normalizedPath = path.front() != '.' ? '.' + path : path;
    Path pathto(normalizedPath);
    // Use normalized path for consistent resolution
    Value value = Resolve(normalizedPath, Value(Json::arrayValue));
    if (!value.isArray())
      value = Value(Json::arrayValue);
    else if (!append && value.size() > 0)
      value.clear();

    UInt32 length = static_cast<UInt32>(Input.size());

    for (UInt32 i = 0; i < length; ++i) {
      value.append(MakeValue<T>(Input[i]));
    }
...
template <> SInt32 ExternalFile::GetPathValue(std::string path, SInt32 defaultValue) {
        ...
        {
                logger::info("Json::LogicError: {}", msg.what());
        }
...
bool ExternalFile::IsForm(const std::string &path) {
  s_dataLock.Enter();
  Value value = Resolve(path);
  bool result = value.isString() && Forms::IsFormString(value.asString());
  s_dataLock.Leave();
  return result;
}

bool ExternalFile::SetRawPathValue(const std::string &path, const std::string &raw) {
  if (path.empty() || raw.empty())
    return false;
  bool parsed = false;
  s_dataLock.Enter();
  ...
  if (!readErrors.empty()) {
    logger::info("JSON PARSER ERROR (SetRawPathValue):");
    logger::info("{}\n", readErrors.c_str());
  }
...
bool UnloadFile(std::string name) {
  std::call_once(s_globalsInitFlag, InitGlobals);
  if (name.empty())
    return false;
  s_loadLock->Enter();
...
bool RevertFile(std::string name, bool savechanges, bool minify) {
  ...
  for (FileVector::iterator itr = s_Files->begin(); itr != s_Files->end(); ++itr) {
    if (boost::iequals(name, (*itr)->name)) {
      ExternalFile *File = (*itr);
      logger::info("JSON Reverted {}", File->name.c_str());
      if (savechanges && File->isModified)
        saved = File->SaveFile(minify);
      delete File; // Fix memory leak
      s_Files->erase(itr);
      break;
    }
  }
...
void SaveFiles() {
  ...
  if (!s_Files->empty()) {
    logger::info("JSON Files: {}", (int)s_Files->size());
    for (FileVector::iterator itr = s_Files->begin(); itr != s_Files->end(); ++itr) {
      if ((*itr)->isModified) {
        logger::info("\tSaving: {}", (*itr)->name.c_str());
        (*itr)->SaveFile();
      }
  ...
  logger::info("JSON SaveFiles - Done");
  s_loadLock->Leave();
}
...
bool ExternalFile::LoadFile() {
  ...
  logger::info("JSON Loading: {}", Path.generic_string().c_str());
  // Check if file exists
  if (!fs::exists(Path)) {
    logger::info("JSON: File does not exist, init empty root object...");
  ...
  if (!parsed || !root.isObject()) {
    logger::info("JSON: Failed to parse file, init empty root object...");
    logger::info("\n");
    root.clear();
    root = Json::objectValue;
    // reader.parse("{}", root, false);
    // Don't set isLoaded on failure - allow retry
  } else {
    isLoaded = true;
  }
...
bool ExternalFile::SaveFile() {
  ...
  if (!isModified || !root.isObject())
    return false;
  logger::info("JSON Saving: {}", name.c_str());
  ...
  try {
    ...
    logger::info("FILESYSTEM EXCEPTION:");
    logger::info("{}\n", ex.what());
    return false;
  }
  ...
  try {
    ...
    logger::info("JSON WRITE EXCEPTION:");
    logger::info("{}\n", ex.what());
    success = false;
  }
...
void ExternalFile::CopyFileTo(std::string copyTo) {
  ...
  if (!boost::iends_with(copyTo, ".json"))
    copyTo += ".json";

  // Get or create the target file first (without holding our lock)
  ExternalFile *target = GetFile(copyTo);
  if (!target)
    return;

  // Use consistent lock ordering (alphabetical by docpath) to prevent deadlock
  // when two threads call fileA.CopyFileTo(fileB) and fileB.CopyFileTo(fileA)
  ICriticalSection *firstLock = &s_dataLock;
  ICriticalSection *secondLock = &target->s_dataLock;
  if (docpath > target->docpath) {
    std::swap(firstLock, secondLock);
  }

  firstLock->Enter();
  secondLock->Enter();

  // Copy the JSON root to the target
  target->root = root; // Deep copy of JSON
